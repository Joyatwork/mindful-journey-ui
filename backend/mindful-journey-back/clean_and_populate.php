...
// Script pour nettoyer et remplir la base de données avec des données réalistes
...
echo "🧹 Nettoyage et remplissage de la base de données...\n\n";

try {
    // OPTION: Nettoyer les données existantes
    echo "🗑️  Nettoyage des données existantes...\n";
    MeditationSession::truncate();
    MoodEntry::truncate();
    
    // Garder seulement Sophie Laurent si elle existe
    $sophie = User::where('name', 'Sophie Laurent')->first();
    $sophieId = $sophie ? $sophie->id : null;
    
    User::where('id', '!=', $sophieId ?? 0)->delete();
    ...
    echo "Maintenant vous avez une application complète avec des données réalistes.\n\n";
