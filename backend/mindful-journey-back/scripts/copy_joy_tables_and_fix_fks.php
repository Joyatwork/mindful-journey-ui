<?php
// Copy tables that exist only in JoyAtWork into mindful_journey and remap user_id via merge_maps.users_map
...
try {
    ...
    if (empty($tables)) {
        echo "No Joy-only tables to copy.\n";
        exit(0);
    }

    echo "Joy-only tables to copy: " . implode(', ', $tables) . "\n";
    ...
    foreach ($tables as $table) {
        ...
        try {
            ...
            // Optionally, detect other *_id columns that reference users (e.g., owner_id) - we skip for now
